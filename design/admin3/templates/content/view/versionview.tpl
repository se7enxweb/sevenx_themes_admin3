                                ...
                                <div class="block">
                                    {if $translation_list|count|gt( 1 )}
                                        ...
                                        {foreach $translation_list as $locale_code}
                                            ...
                                            <p>
                                                <input type="radio" name="SelectedLanguage" value="{$locale_code}"
                                                    {if eq( $locale_code, $object_languagecode )}checked="checked" {/if} />
                                                {if $locale_object.is_valid}
                                                    <img src="{$locale_code|flag_icon}" width="18" height="12" alt="{$locale_code}"
                                                        style="vertical-align: middle;" />
                                                    {$locale_object.intl_language_name|shorten( 16 )}
                                                {else}
                                        ...
                                        {/foreach}
                                    {else}
                                        <input type="radio" name="SelectedLanguage"
                                            value="{$version.language_list[0].language_code}" checked="checked"
                                            disabled="disabled" />
                                        {if $version.language_list[0].locale.is_valid}
                                            <img src="{$version.language_list[0].language_code|flag_icon}" width="18" height="12"
                                                alt="{$version.language_list[0].language_code}" style="vertical-align: middle;" />
                                            {$version.language_list[0].locale.intl_language_name|shorten( 16 )}
                                        {else}
                            ...
                            {section show=$version.node_assignments|count|gt( 0 )}
                                ...
                                <div class="block">
                                    {section show=$version.node_assignments|count|gt( 1 )}
                                        {section var=Locations loop=$version.node_assignments}
                                            <p>
                                                <input type="radio" name="SelectedPlacement" value="{$Locations.item.id}"
                                                    {if eq( $Locations.item.id, $placement )}checked="checked"
                                                    {/if} />&nbsp;{$Locations.item.parent_node_obj.name|wash}
                                            </p>
                                        {/section}
                                        {section-else}
                                            <p>
                                                <input type="radio" name="SelectedPlacement"
                                                    value="{$version.node_assignments[0].id}" checked="checked"
                                                    disabled="disabled" />&nbsp;{$version.node_assignments[0].parent_node_obj.name|wash}
                                            </p>
                                        {/section}
                                    </div>
                                {/section}
                                ...
                                <div class="block">
                                    {if $site_access_locale_map|count|gt( 1 )}
                                        {foreach $site_access_locale_map as $related_site_access => $related_site_access_locale}
                                            <p>
                                                <input type="radio" name="SelectedSiteAccess" value="{$related_site_access}"
                                                    {if eq( $related_site_access, $siteaccess )}checked="checked"
                                                    {/if} />&nbsp;{$related_site_access|wash}
                                            </p>
                                        {/foreach}
                                    {else}
                                        <p>
                                            <input type="radio" name="SelectedSiteAccess" value="{$site_designs[0]}"
                                                checked="checked" disabled="disabled" />&nbsp;{$site_designs[0]|wash}
                                        </p>
                                    {/if}
                                </div>
                                ...
                                <div class="block">
                                    <input class="button" type="submit" name="ChangeSettingsButton"
                                        ...
                                        title="{'View the version that is currently being displayed using the selected language, location and design.'|i18n( 'design/admin/content/view/versionview' )}" />
                                </div>
                ...
                {if $object_languagecode|eq( $site_access_locale_map[$siteaccess] )|not}
                    <div class="message-feedback">
                        <h2><span class="time">[{currentdate()|l10n( shortdatetime )}]</span>
                            {'Translation mismatch'|i18n( 'design/admin/content/view/versionview' )}</h2>
                        <p>{'Your selected translation does not match the language of your selected siteaccess. This may lead to unexpected results in the preview, however it may also be what you intended.'|i18n( 'design/admin/content/view/versionview' )}
                        </p>
                    </div>
                {/if}
                        ...
                        <div class="box-content">
                            ...
                            <div class="context-information">
                                ...
                                <p class="right translation">
                                    {$object_languagecode|locale().intl_language_name} <img
                                        src="{$object_languagecode|flag_icon}" width="18" height="12"
                            ...
                            <div class="mainobject-window">
                                ...
                                <iframe
                                    ...
                                    Your browser does not support iframes. Please see this <a
                                        href={concat("content/versionview/",$object.id,"/",$view_version.version,"/",$language, "/site_access/", $siteaccess)|ezurl}>link</a>
                                    instead.
                                </iframe>
   ...
   object.status 2 is archived *}
                          ...
                          and( eq( $object.status, 2 ), $object.can_edit ) )}
                                        <input class="defaultbutton" type="submit" name="EditButton"
                                            value="{'Back to edit'|i18n( 'design/admin/content/view/versionview' )}"
                                            title="{'Edit the draft that is being displayed.'|i18n( 'design/admin/content/view/versionview' )}" />
                                    ...
                                    {else}
                                        <input class="button-disabled" type="submit" name="EditButton"
                                            value="{'Back to edit'|i18n( 'design/admin/content/view/versionview' )}"
                                            disabled="disabled"
                                            title="{'This version is not a draft and therefore cannot be edited.'|i18n( 'design/admin/content/view/versionview' )}" />
                                        <input class="button-disabled" type="submit" name="PreviewPublishButton"
                                            value="{'Publish'|i18n( 'design/admin/content/view/versionview' )}"
                                            disabled="disabled"
                                            title="{'Publish the draft that is being displayed.'|i18n( 'design/admin/content/view/versionview' )}" />
